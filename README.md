...
Вы можете смотреть и разбирать примеры (нэймспейс `Example`), но **не копируйте** код примеров напрямую, не разбирая. Копирование кода примеров будет отрицательно влиять на итоговый результат.
...
Вы **можете** использовать готовые реализации контейнера зависимостей, роутера, классов запроса и ответа и всего, что относится к приложению в целом, в виде composer-зависимостей. Вы так же **можете** не реализовывать веб-интерфейс, но тогда реализуйте запуск из консоли. 
...
Основная ваша задача — создать систему получения данных из различных источников в ваши классы. То есть если у вас в итоге будет _репозиторий_, из которого можно получить данные в виде конкретных классов, задание будет считаться выполненым. Остальное (собственная реализация request / response, используемые пакеты для них, отображение и прочее) будет дополнительным плюсом. Но **реализация получения данных должна быть полностью вашей**.

Расположение файлов данных относительно корня проекта будет постоянным, но только на время жизни проекта. Иными словами, пути к файлам данных должны быть указаны **один** раз в конфигурации (или при инициализации) приложения.
